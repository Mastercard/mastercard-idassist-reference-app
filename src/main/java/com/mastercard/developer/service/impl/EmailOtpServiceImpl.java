...
public class EmailOtpServiceImpl implements EmailOtpService {
    ...
    @Autowired
    public EmailOtpServiceImpl(EmailOtpApi otpApi, ExceptionUtil exceptionUtil) {
        this.otpApi = otpApi;
        this.exceptionUtil = exceptionUtil;
    }

    @Override
    public Otp createEmailOtp(EmailOtp emailOtp) {
        try {
            return otpApi.sendEmailOtp(emailOtp, encryptionEnabled);
    ...
    public OtpVerificationResult createVerifyOtp(OtpVerification otpVerification) {
        try {
            return otpApi.verifyEmailOtp(otpVerification, encryptionEnabled);
        } catch (ApiException e) {
            throw exceptionUtil.logAndConvertToServiceException(e);
        }
    }
